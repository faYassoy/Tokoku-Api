...
<body>
    <section style="padding: 0.5cm; border: 1px solid black;">
        ...
        <div class="list-content" style="margin-top: 0.5cm">
            <p class="py-1">Nomor Pembelian : {{ $wholesale->delivery_number }}</p>
            <p class="py-1">Tanggal : {{ date('d-m-Y', strtotime($wholesale->created_at)) }}</p>
            <p class="py-1">Tanggal Cetak : {{ date('d-m-Y') }}</p>
            <p class="py-1">Gudang : {{ $wholesale->warehouse->name }}</p>
            <p class="py-1">Supplier : {{ $wholesale->supplier->name }}</p>
            <p class="py-1">Nomor LPB : {{ $wholesale->no_lpb }}</p>
            <p class="py-1">Nomor Faktur : {{ $wholesale->no_faktur }}</p>
            <p class="py-1">Catatan : {{ $wholesale->note }}</p>
        </div>

        <p style="margin-top: 32px;">Membeli barang berikut : </p>
        ...
        <div style="margin-top: 12px;margin-bottom: 32px;">
            <table class="table">
                ...
                @php
                    $i=1;
                @endphp
                @forelse ($wholesale->wholesale_products as $wholesaleProduct)
                <tr>
                    <td>{{ $i++ }}</td>
                    <td>
                        {{ $wholesaleProduct->product?->name }}
                        <p style="font-size: 10px;">({{ $wholesaleProduct->product?->code }})</p>
                    </td>
                    <td>
        ...
        <div class="list-content" style="margin-top: 0.5cm">
            ...
            <p class="py-1">Ongkir : {{ number_format($wholesale->delivery_fee , 0, ',', '.') }}</p>
            <p class="py-1">Pajak : {{ number_format($wholesale->tax , 0, ',', '.') }}</p>
            <p class="py-1">Biaya Lainya : {{ number_format($wholesale->other_fee , 0, ',', '.') }}</p>
            <p class="py-1">Total : {{ number_format($wholesale->total_price , 0, ',', '.') }}</p>
        </div>
